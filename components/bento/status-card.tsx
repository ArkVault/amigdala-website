...
const REGIONS = [
  { name: "CRM", status: "SCOPED", signal: "Sales context" },
  { name: "ERP", status: "SCOPED", signal: "Operating reality" },
  { name: "TEAMS", status: "SCOPED", signal: "Escalations" },
  { name: "TICKETS", status: "SCOPED", signal: "Decision evidence" },
]
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 flex flex-col p-4 gap-0">
        ...
        <div className="grid grid-cols-3 gap-2 border-b border-border pb-2 mb-2">
          ...
          <span className="text-[9px] tracking-[0.15em] uppercase text-muted-foreground text-right">Signal</span>
        </div>
        {REGIONS.map((region) => (
          <div
            key={region.name}
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span
                ...
                style={{
                  backgroundColor: region.status === "SCOPED" ? "#ea580c" : "hsl(var(--muted-foreground))",
                }}
              />
              <span className="text-xs font-mono text-muted-foreground">{region.status}</span>
            </div>
            <span className="text-xs font-mono text-foreground text-right">{region.signal}</span>
          </div>
        ))}
